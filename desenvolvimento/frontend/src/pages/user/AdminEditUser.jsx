...
export default function AdminEditUser() {
  ...
  const [formData, setFormData] = useState({
    ...
    matricula: userToEdit?.matricula || '',
    email: userToEdit?.email || '',
    id_patente: userToEdit?.id_patente || '', // Novo campo para a patente
    password: '' // A senha sempre começa vazia
  });

  const [status, setStatus] = useState({ type: '', message: '' });
  const [patentes, setPatentes] = useState([]); // Novo estado para armazenar as patentes
  ...
  // Novo useEffect para buscar as patentes do backend
  useEffect(() => {
    const fetchPatentes = async () => {
      try {
        ...
        setStatus({ type: 'error', message: 'Erro ao carregar a lista de patentes.' });
      }
    };
    fetchPatentes();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Certifique-se de que id_patente está incluído no formData
      await api.put('/admin/updateuser', formData);
