...
// -----------------------------------------------------------------------
// SUBSTITUIÇÃO PONTUAL (exceção só pra um dia+turno específico, sem
// mexer no vínculo mensal de ninguém)
// -----------------------------------------------------------------------

// POST /escalas/substituicao/adicionar
// Adiciona um militar à escala apenas naquele dia
router.post(
...
// Exclui um militar da escala apenas naquele dia
router.post(
...
// Permuta dois militares apenas naquele dia
router.post(
...
// Lista as substituições pontuais registradas pra uma data (formato
// aceito pelo mesmo validador de data usado no resto do módulo)
router.get(
...
// Reverte uma substituição pontual (apaga a exceção, volta ao vínculo
// mensal normal daquele dia)
router.delete(
  "/substituicao/:id",
  verifyJwt,
  isAdmin,
